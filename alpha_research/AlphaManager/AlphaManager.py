...
class AlphaManager:
    ...
    def query_alpha(self, db: str, collection: str, query: dict):
        try:
            ...
            for res in results:
                del res['_id']
                alpha_list.append(self.from_dict_to_alphaStorage(res))
            return alpha_list
        except Exception:
            traceback.print_exc()
    ...
    def dash_webpage(self):
        ...
        app.layout = html.Div(children=[
            ...
            html.Div([
                ...
                html.Div(['IP  ', dcc.Input(id='ip_address', value='', type='text'),
                          'Port  ', dcc.Input(id='port_number', value='', type='number')]),
                html.Div(['User  ', dcc.Input(id='username', value='', type='text'),
                          'Possword  ', dcc.Input(id='password', value='', type='password')]),
            ...
            html.Div([
                # dash_table.DataTable(id='display-table'),
                html.Pre(id='query-result'),
                html.Pre(id='query-result2'),

            ], style={'float': 'right', 'width': '50%', 'overflow': 'auto', 'height': '700px'}
            ),
        ...
        def query_by_idea(n_clicks_idea, n_clicks_name, db, collection, alpha_idea, alpha_name):
            changed_id = [p['prop_id'] for p in dash.callback_context.triggered][0]

            if 'query_by_idea_button' in changed_id:
                result = self.manager.query_alpha_by_alpha_idea(db, collection, alpha_idea)
                res_str = 'query_by_idea\n'
                for re in result:
                    res_str += json.dumps(re.to_dict(), default=json_util.default, indent=2)
                    res_str += '\n'
                    res_str += '----------------------------------\n'
                return res_str

            if 'query_by_name_button' in changed_id:
                result = self.manager.query_alpha_by_alpha_name(db, collection, alpha_name)
                res_str = 'query_by_name\n'
                for re in result:
                    res_str += json.dumps(re.to_dict(), default=json_util.default, indent=2)
                    res_str += '\n'
                    res_str += '----------------------------------\n'
...
if __name__ == '__main__':
    ...
    # connect mongodb
    AlphaManagerPage().dash_webpage().run_server(host='127.0.0.1', debug=True)
