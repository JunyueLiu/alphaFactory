...
def alpha_21(df:pd.DataFrame):
    ...
    factor = np.where(condition1 < condition2, -1,np.where(condition2<condition3,1,np.where(condition4>=1,1,-1)))
    print(factor)
    return factor
...
def alpha_54(df:pd.DataFrame):
    factor = ( -1 * ((df['low'] - df['close'])* np.power(df['open'],5)) / ((df['low'] - df['high']) * np.power(df['close'],5)))
    #print(factor)
    return factor
